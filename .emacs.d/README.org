...
* Delight
Use-package has support for both diminish and delight, packages used for hiding minor-mode lighters from the modeline. I prefer delight as the documentation is better and diminish failed me. I may one day switch to minions to hide all lighters behind an icon. Or use mood-line or doom modeline.
#+BEGIN_SRC emacs-lisp :tangle init.el
(use-package delight)
#+END_SRC
