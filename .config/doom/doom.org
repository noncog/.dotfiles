...
*** Hide Private Nodes in org-roam-node-find

By using advice, we can filter the arguments passed to org-roam-node-find. Here, we make a function which accepts all the same arguments as org-roam-node-find, and simply return a list of our preferred values instead.

This may need some work eventually to prevent the loss of arguments when used in other cases by passing along any previously given. For now, this works fine.
