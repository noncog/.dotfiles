...
Increase the count. I use this to be able to move back and forth between more help windows using the keybinds I set.
...
By default, Doom centers the page you visit after opening it. Often, I can't see the information behind the helpful window, so I remove the old advice and update it.
...
Doom lacks the ability to move through previously viewed pages in the helpful buffer. So, we can add them using winner mode. These keybinds fit well with the rest and make sense logically.
...
Doom typically sets your Dashboard directory to latest of the recent file list. It's supposed to be convenient for when you accidentally close all buffers but want to resume work on a project. This rarely happens, and the for the rest of the time, this behavior is inconvenient. Let's remove it.
...
I like to use by history first and then alphabetically.
...
This adds all of the following:
...
*** Testing Fix for Doom User

#+begin_src emacs-lisp :tangle no :noweb-ref magit-disabled
