...
** :small_red_triangle_down: [[https://orgmode.org/][Org]]
...
According to Doom, org's directory must be set early due to it's loading process. Therfore it's not inside the use-package configuration.

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-org-init
(setq org-directory "~/documents/org/")
#+end_src

**** Brain Buffer Viewer :TODO:

This function gives me access to my primary org document anywhere in Emacs.

It opens the file if it isn't already opened and displays it in a side window. Because it's a toggle, it also closes the window and buffer upon repeating the command.
...
#+begin_src emacs-lisp :tangle no :exports none :noweb-ref use-package-org-config
...
** :small_red_triangle_down: Org-Agenda
...
** :pencil: [[https://github.com/weirdNox/org-noter][Org-Noter]]
...
** :chart-with-upwards-trend: [[https://orgmode.org/manual/Org-Plot.html][Org-Plot]]
...
Also known as Org Export. It controls the exporting of org documents.
...
** :book: [[https://github.com/vedang/pdf-tools][PDF-Tools]]
...
** Electric Indent
I've been having issues with electric indent's behavior when editing shell scripts...

So we disable it for that mode.
...
Developing convetional commit/angular linting integration for Magit. Base on this [[https://www.adventuresinwhy.com/post/commit-message-linting/][this]] post.
