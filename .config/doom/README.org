...
According to Doom, org's directory must be set early due to it's loading process. Therefore it's not inside the use-package configuration.
...
The number of lines relative to the top and bottom of the screen, at which the buffer should scroll. This provides room to see what is coming and prevents you from passing up your desired information.
...
I only set a couple of global keybinds here that have no other relevant place, like in a package's settings.
...
This section is obviously dedicated to configuring packages.

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-end :exports none
)
#+end_src
...
Often, users struggle to define org mode. Increasingly, people are finding more uses and interactions for it, rendering it even harder to thoroughly define. I aim to do the same.
...
#+begin_src emacs-lisp :tangle no :noweb-ref use-package-org-init
:init
#+end_src

*** Brain Buffer Viewer

This function gives me access to my primary org document (brain.org) anywhere in Emacs.

It toggles open/closed the brain.org file/buffer and it's popup side window.
...
Next, I define a function to open the file/buffer and display it in a window.
...
Then, I use Doom's popup windowing to control how the buffer is displayed.

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-org-init
(set-popup-rule! "^brain.org" :side 'right :vslot -1 :width 70 :modeline t :select t :quit nil)
#+end_src

Finally, I set a keybind or two to call my toggle-brain function.
...
Eventually, I think that I will turn this function into a universal file displaying function or something...
...
#+begin_src emacs-lisp :tangle no :noweb-ref use-package-org-agenda-init
:init
#+end_src

*** Agenda Launcher

I define a function to call my custom agenda either from M-x or through a keybind.
...
*** Agenda Popup Window Settings

Doom provides pop-up windowing rules to control how select buffers are displayed.

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-org-agenda-config
(set-popup-rule! "^*Org Agenda*" :side 'right :vslot 1 :width 67 :modeline nil :select t :quit t)
#+end_src

*** Custom Agenda Section Title Face

I make the agenda section titles slightly bigger and bold to more easily differentiate them visually.
...
This is my custom agenda view. It has everything I want and nothing more.

I see no need for another agenda package, consider writing your own!
...
I call this function when displaying my custom agenda to remove empty sections.
...
I had issues with the way Doom and org-noter close pdf buffers without returning my windowing back to normal. This function solves that.
...
#+begin_src emacs-lisp :tangle no :noweb-ref use-package-ox :exports none
(use-package! ox
#+end_src

** Config

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-ox-config
:config
#+end_src
...
#+begin_src emacs-lisp :tangle no :noweb-ref use-package-ox-latex
(use-package! ox-latex
#+end_src

** Init

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-ox-latex-init
:init
#+end_src
...
* Consult :DESCRIPTION:LINK:

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-consult :exports none
(use-package! consult
#+end_src

** Config

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-consult-config
:config
#+end_src

*** Org Heading Unfolding

This fixes a bug where consult searches can't display the heading you want, only the nearest visible to it.

Notice that =org-fold-reveal= accepts an argument. This changes the way the org-headings around the search result are displayed.
...
Are a better package of tools for viewing and interacting with PDFs. Also, a requirement to use org-noter.

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-pdf-tools :exports none
(use-package! pdf-tools
#+end_src

** Config

#+begin_src emacs-lisp :tangle no :noweb-ref use-package-pdf-tools-config
:config
#+end_src
...
(after! pdf-tools
  <<use-package-pdf-tools>>
    <<use-package-pdf-tools-config>>
    <<use-package-end>>
    )
#+end_src

* Pulsar :TODO:DESCRIPTION:LINK:
...
* Ace Window :TODO:DESCRIPTION:LINK:
...
* Projectile :TODO:DESCRIPTION:LINK:
...
* Visual Fill Column :TODO:LINK:DESCRIPTION:
...
* Yasnippet :TODO:DESCRIPTION:DESCRIPTION:
** Information
...
* Electric Indent :DESCRIPTION:LINK:
** Disable For Modes

I've been having issues with electric indent's behavior when editing shell scripts... So I disable it for that mode.
...
* Magit :LINK:DESCRIPTION:

Developing conventional commit/angular linting integration for Magit. Base on this [[https://www.adventuresinwhy.com/post/commit-message-linting/][this]] post.
