...
I typically use the [[https://draculatheme.com/][Dracula]] theme, but I'm experimenting with Doom Vibrant/One.
...
Right now, I prefer to use my Bash configuration instead of Zsh, so we set Vterm to that when we're on macOS.
...
You can load it in Doom by enabling the =wordwrap= module.
