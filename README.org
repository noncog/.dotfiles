...
Otherwise, to install dotfiles managed with a bare repository on a fresh system do the following:
** Simple Clone of dotfiles
#+BEGIN_SRC
  git clone --bare https://github.com/noncog/.dotfiles $HOME/.dotfiles
  # alias only in this shell instance
  # assumes that you back up your .bashrc where the permanent alias is,
